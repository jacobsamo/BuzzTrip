...
import { Search, MapPin, Users, Globe, Lock, EyeOff, Calendar, ArrowUpDown, ArrowUp, ArrowDown, ChevronLeft, ChevronRight } from "lucide-react"
...
export function MapsListTable({
  ...
  const columns: ColumnDef<MapWithStats>[] = useMemo(() => [
    ...
    },
    {
      ...
      header: ({ column }) => {
        if (!enableSorting) return <div className="text-center">Collaborators</div>
        const isSorted = column.getIsSorted()
        return (
          <div className="flex justify-center">
            <Button
  ...
  ], [enableSorting, showActions, router, getVisibilityVariant, getVisibilityIcon])
  ...
  const displayMaps = enableSorting || enablePagination
      ...
      {showSearch && (
        <div className="flex items-center gap-4">
          <div className="relative flex-1 max-w-sm">
            ...
            <Input
              placeholder={enableSorting ? "Filter by map title..." : "Search maps by title, description, or owner..."}
              value={enableSorting
      ...
      {displayMaps.length > 0 ? (
        <>
              ...
              {displayMaps.map((map) => (
                ...
                >
                  <CardContent className="p-4 space-y-3">
                    ...
                    <div>
                      <h3 className="font-semibold text-foreground mb-1">{map.title}</h3>
                      {map.description && (
                    ...
                    <div className="grid grid-cols-2 gap-3 pt-2 border-t border-border">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm font-medium text-foreground">{map.markersCount}</span>
                      </div>
                      <div className="flex items-center gap-2">
                        <Users className="h-4 w-4 text-muted-foreground" />
                        <span className="text-sm text-muted-foreground">Collaborators:</span>
                        <span className="text-sm font-medium text-foreground">{map.collaboratorsCount}</span>
                      </div>
                    </div>
          ...
          <div className="hidden md:block">
            ...
            >
              <Table>
                <TableHeader className={enablePagination ? "bg-muted/50" : "sticky top-0 bg-muted/95 backdrop-blur z-10"}>
                  {enableSorting || enablePagination ? (
                    table.getHeaderGroups().map((headerGroup) => (
                      <TableRow key={headerGroup.id} className={enablePagination ? "hover:bg-muted/50" : ""}>
        ...
        <div className="py-12 text-center text-muted-foreground border border-border rounded-md">
          {searchQuery || (enableSorting && table.getColumn("title")?.getFilterValue()) ? (
            <div>
